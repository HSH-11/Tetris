...
const int group_size = 10*sizeof(char);
string key_group;
...
char getTetrisKey(TetrisState state, bool fromUser, bool toFile) {
  ...
  if (fromUser == true) {
  ...
  else { // fromUser == false //relplay 모드
    ...
    if (infStream.eof() == true){//버퍼에 10개가 차지 않는 경우
      if(infStream.gcount() == last_key_count)//파일에서 읽어온 개수만큼만 동작
        key = 'q';
      else{
        key = key_group[0];
        key_group.erase(0, 1);
      ...
      if (key_group.empty() || key_group.size() < 1) {
        key_group.resize(group_size);
        infStream.read(&key_group[0],key_group.size());
      }
      key = key_group[0];
      key_group.erase(0, 1);
    }
  ...
  if (toFile == true) {//record mode
    ...
    //outfStream << key;
    key_group.push_back(key);
    if (key_group.size()==group_size){
        outfStream.write(key_group.c_str(),key_group.size());
        key_group.clear();
    }
...
int main(int argc, char *argv[]) {
  ...
  if (!key_group.empty()){//버퍼에 아직 key가 남은 경우 나머지 key들을 파일에 기록
    outfStream.write(key_group.c_str(),key_group.size());
    key_group.clear();
  }
