...
int Tetris::findOpIdxByKey(char key) {//key값 목록
...
void Tetris::setOperation(char key, TetrisState s0, ActionHandler *h1,
  ...
  else {//key에 해당하는 idx가 없어 -1이 return된 상황
    if (nOps == MAX_TETRIS_OPERATIONS) {
      cerr << "Tetris::operations[] is full." << endl;
      return;
    }
    operations[nOps] = new TetrisOperation(key, s0, h1, s1, h2, s2);//새로운 key에 대한 operation할당
    nOps++;
  }
}
// Tetris::setOperation('key', TetrisState::실행조건, key 행위, TetrisState::key행위 후 상태, counter 행위, TetrisState::counter행위 후 상태);
void Tetris::setDefaultOperations(void) {
  ...
  setOperation(' ', TetrisState::Running, new OnDown() , TetrisState::Running, new OnUp(), TetrisState::NewBlock);
